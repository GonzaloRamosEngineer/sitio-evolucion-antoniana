...
import { ArrowLeft, Facebook, Twitter, Linkedin, Copy, Check, Calendar } from 'lucide-react';
  ...
  if (!item) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center bg-brand-sand px-4">
        <h1 className="text-3xl font-bold text-brand-dark mb-4 font-poppins">Noticia no encontrada</h1>
        <p className="text-gray-600 mb-8">La noticia que buscas no existe o ha sido eliminada.</p>
        <Link to="/novedades">
            <Button className="bg-brand-primary hover:bg-brand-dark text-white">Volver a Novedades</Button>
        </Link>
      </div>
    );
  }
            ...
            <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-white/50">
              ...
              <div className="p-6 md:p-12">
                ...
                <div className="flex items-center gap-2 text-brand-gold font-bold text-xs uppercase tracking-wider mb-4">
                  ...
                  <span>{new Date(item.created_at).toLocaleDateString('es-AR', { dateStyle: 'long' })}</span>
                </div>
                ...
                <div className="mt-16 pt-8 border-t border-gray-100">
                  ...
                  <div className="flex flex-wrap justify-center gap-3">
                    <a href={facebookHref} target="_blank" rel="noopener noreferrer">
                      <Button variant="outline" size="sm" className="rounded-full hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all">
                        <Facebook className="h-4 w-4 mr-2" /> Facebook
                      </Button>
                    </a>
                    <a href={twitterHref} target="_blank" rel="noopener noreferrer">
                      <Button variant="outline" size="sm" className="rounded-full hover:bg-sky-50 hover:text-sky-500 hover:border-sky-200 transition-all">
                        <Twitter className="h-4 w-4 mr-2" /> Twitter (X)
                      </Button>
                    </a>
                    <a href={linkedinHref} target="_blank" rel="noopener noreferrer">
                      <Button variant="outline" size="sm" className="rounded-full hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200 transition-all">
                        <Linkedin className="h-4 w-4 mr-2" /> LinkedIn
                      </Button>
                    </a>

                    <a href={whatsappHref} target="_blank" rel="noopener noreferrer">
                      <Button variant="outline" size="sm" className="rounded-full hover:bg-green-50 hover:text-green-600 hover:border-green-200 transition-all">
                        <svg viewBox="0 0 32 32" width="16" height="16" className="mr-2 fill-current" aria-hidden="true">
                    ...
                    <Button
                      ...
                      className={`rounded-full transition-all border-gray-200 ${copied ? 'bg-green-50 border-green-200 text-green-700' : 'hover:bg-gray-50'}`}
                    >
                      <AnimatePresence mode="wait" initial={false}>
                        {copied ? (
                          <motion.span key="check" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="inline-flex items-center">
                            <Check className="h-4 w-4 mr-2" /> Copiado
                          </motion.span>
                        ) : (
                          <motion.span key="copy" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="inline-flex items-center">
