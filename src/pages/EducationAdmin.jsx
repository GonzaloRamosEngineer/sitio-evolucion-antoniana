...
const EducationAdmin = () => {
  ...
  const exportToExcelFriendly = () => {
    if (filteredList.length === 0) {
      toast({ title: "Sin datos", description: "No hay registros para exportar con los filtros actuales." });
      return;
    }
...
const MetricCard = ({ title, value, icon: Icon, color }) => {
  ...
  return (
    <Card className={`border border-white/50 shadow-lg ${colors[color]} rounded-[2rem] p-6 flex items-center justify-between transition-all hover:-translate-y-1`}>
      <div className="space-y-1">
        <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">{title}</p>
        <p className="text-4xl font-black text-brand-dark tracking-tighter">{value}</p>
      </div>
      <div className={`p-4 rounded-2xl bg-white shadow-sm border border-inherit`}><Icon size={24} /></div>
    </Card>
  );
};
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <Badge className={`${config.class} border-none text-[9px] font-black px-3 py-1 flex items-center gap-2 w-fit tracking-widest shadow-sm`}>
