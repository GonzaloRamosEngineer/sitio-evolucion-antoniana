...
- **Caché de estado servidor — TanStack Query** (migración incremental, ROADMAP 4.2): las páginas públicas (Home, Activities, NewsPage, PartnersPage, BenefitsPage) leen vía `src/hooks/useContentQueries.js`; el resto todavía usa `useEffect` manual. Reglas:
